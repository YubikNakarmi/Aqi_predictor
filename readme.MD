...
[![Push to Docker Hub Dev Branch](https://github.com/YubikNakarmi/Aqi_predictor/actions/workflows/push_images.yml/badge.svg?branch=dev)](https://github.com/YubikNakarmi/Aqi_predictor/actions/workflows/push_images.yml)
[![Push to Docker Hub Test Branch](https://github.com/YubikNakarmi/Aqi_predictor/actions/workflows/push_images.yml/badge.svg?branch=test)](https://github.com/YubikNakarmi/Aqi_predictor/actions/workflows/push_images.yml)
[![Terraform Apply](https://github.com/YubikNakarmi/Aqi_predictor/actions/workflows/terraform_apply.yml/badge.svg?branch=main&event=workflow_dispatch)](https://github.com/YubikNakarmi/Aqi_predictor/actions/workflows/terraform_apply.yml)
...
![Pipeline Architecture](.github_utils/architecture.png)
...
| `monitor_dag` | @weekly |  | Placeholder for future monitoring tasks |
...
|---|---| 
|`xgb_{TARGET_COL}_hourly_tuning`|Optuna tuning runs for each horizon (1–24h)|
|`xgb_{TARGET_COL}_hourly_training`|Training runs for each horizon (1–24h)|
|`xgb_{TARGET_COL}_hourly_evaluation`|Evaluation runs for each horizon (1–24h) and aggregate metrics|
|`xgb_{TARGET_COL}_hourly_prediction`|Daily prediction runs with the latest serving model|
|`xgb_{TARGET_COL}_hourly_serving `|Model registry entries for the composite `ServeModel`|

TARGET_COL is set in `.env` (default `pm25`), so experiment names will be `xgb_pm25_hourly_tuning`, etc.
## Models
...
- Uses Azure service principal secret authentication in GitHub Actions

Required GitHub secrets for `terraform_apply.yml`:
- `AZURE_CLIENT_ID`
- `AZURE_CLIENT_SECRET`
- `AZURE_TENANT_ID`
...
terraform init
terraform plan 
terraform apply var="public_key=${{public_key.pem }}"
```

If you want to use OIDC instead of a client secret, you must create a federated identity credential on the Azure app or managed identity first. Without that Azure-side setup, the GitHub Actions login step will fail with `AADSTS70025`.
...
Python lint checks with pyling on push and merge to main. Unit tests for data cleaning and model training modules.

#### Docker image build and push on release
 ![Docker_push](.github_utils/dockerpush.drawio.png)
 Pushes latests images by tagging based on branch and commit SHA. Images are pushed to Docker Hub under `yubiknakarmi/aqi-prediction` and `yubiknakarmi/aqi-trainer`.

#### Ml tests and CML reports
 Ml tests runs on merge to main/dev and on pull request. It runs a subset of the training pipeline with a small sample of data to verify that the training process completes successfully and that metrics are within expected ranges. CML is used to generate reports with training metrics and plots, which are then posted as comments on the pull request for easy review. 
  
#### Pulling reposotiry updade from github to azure vm (CD)
![CD](.github_utils/CD.png)
Simple deployment workflow that pulls the latest repo from Main and runs a script in Azure VM to update the code and restart the docker compose services. 

#### Terraform Apply Workflow
![Pipeline Architecture](.github_utils/ci_cd(terraform).drawio.png)
This workflow runs through manual trigger and asks for ainput to whether to apply the terraform plan or not. If the user approves, it applies the terraform configuration to provision infrastructure in Azure. This workflow requires the following secrets to be set in GitHub:
- `AZURE_TENANT_ID`
- `AZURE_SUBSCRIPTION_ID`
- `AZURE_CLIENT_ID`(service principal app ID)

> [!NOTE]
> Terrafom Apply and CD into Azure VM couldn't be tested due to restrictions in Azure student account. These workflows expect an RBAC role with permissions to manage infrastructure and deploy to the VM. For local testing, you can comment out the Terraform and deployment steps in the workflow files.
...
- Implement retraining data pipeline using data from AQI API calls
# Credits
